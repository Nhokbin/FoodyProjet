...
public class PresenterLogicItem implements IPresenterItem {
    ...
    public void getListItemByCategory(int categoryId) {
        List<Item> items = modelItem.getListItemByCategory(database, categoryId, 1);
        if (items.size() > 0) {
    ...
    public void getListItemByType(int categoryID, int typeId) {
        List<Item> items = modelItem.getListItemByType(database, categoryID, typeId, 1);
        if (items.size() > 0) {
    ...
    public void getListItemByAddress(int[] id) {
        List<Item> items;
        items = modelItem.getListItemByAddress(database, id[0], id[1], id[2], 1);
    ...
    private List<Item> getListLoadMore(int[] id, int index, final FrameLayout frameProgress, int by) {
        ...
        if (by == LOAD_MORE_BY_CATE) {
            items = modelItem.getListItemByCategory(database, id[0], index);
        } else if (by == LOAD_MORE_BY_TYPE) {
            items = modelItem.getListItemByType(database, id[0], id[1], index);
            Log.d("KTLoad", items.size() + "");
        } else {
